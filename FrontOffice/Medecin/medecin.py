...
def search_doctors():
    v = request.args.get("v", "").strip('"')  # Remove extra quotes
    translator_fr = Translator(to_lang="fr")
    ...
    listDocs = listDoctors()
    if not listDocs["success"]:
        return jsonify({"success": False, "msg": listDocs["msg"]})

    fetch = []
    v = v.lower()
    print(v)
    for doc in listDocs["msg"]:
        if (
            v in (doc[1] or "").lower() or
            v in (doc[2] or "").lower() or
            v in (doc[3] or "").lower() or
            v in (doc[5] or "").lower() or 
            v in (doc[6] or "").lower()

        ):
            fetch.append(doc)
        print(" la specialite est")
        print (doc[6])
